...
export function moveTaskWithinProjectColumn(taskId, symbol, biggerThan, lessThan, row, projectColumnId) {
    return moveTaskWithinColumn(taskId, symbol, biggerThan, lessThan, row, projectColumnId, TASK_TABLE)
}

export function moveTaskToEndOfNewProjectColumn(newRow, newProjectColumnId, taskId, oldProjectColumnId, currentRow, maxRow) {
    return moveTaskToEndOfNewColumn(newRow, newProjectColumnId, taskId, oldProjectColumnId, currentRow, maxRow, TASK_TABLE)
}

export function moveTaskToNewProjectColumn(taskId, oldProjectColumnId, oldRow, newProjectColumnId, newRow, maxRowCurrentColumn, maxRowNewColumn) {
    return moveTaskToNewColumn(taskId, oldProjectColumnId, oldRow, newProjectColumnId, newRow, maxRowCurrentColumn, maxRowNewColumn, TASK_TABLE)
}

async function moveTaskWithinColumn(taskId, symbol, biggerThan, lessThan, row, projectColumnId, activeTable) {
...
async function moveTaskToEndOfNewColumn(newRow, newProjectColumnId, taskId, oldProjectColumnId, currentRow, maxRow, activeTable) {
...
async function moveTaskToNewColumn(taskId, oldProjectColumnId, oldRow, newProjectColumnId, newRow, maxRowCurrentColumn, maxRowNewColumn, activeTable) {
    const { rowColumn, taskIdColumn, columnIdColumn } = columnKeys[activeTable]
    transactionQuery(async (conn) => {
        await disableCurrentTask(conn, taskId, activeTable)
