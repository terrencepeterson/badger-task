...
export async function moveTaskToNewColumn(taskId, oldProjectColumnId, oldRow, newProjectColumnId, newRow, maxRowCurrentColumn, maxRowNewColumn) {
    transactionQuery(async (conn) => {
        ...
        if (oldRow !== (maxRowCurrentColumn - 1)) { // if at end of oldColumn no need to move any tasks
            await moveRows(conn, oldRow, maxRowCurrentColumn, taskId, oldProjectColumnId, '-') // move roms from old column
        }

        if ((maxRowNewColumn) !== newRow) { // if at end of the newColumn no need to move any tasks
            await moveRows(conn, newRow -1, maxRowNewColumn, taskId, newProjectColumnId, '+') // move rows in new column
        }
